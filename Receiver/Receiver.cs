 ...
 * --------------------
  * ver 2.3 : 18 Nov 2015
 * - Modified service action to measure the latency of clients
 * ver 2.2 : 08 Nov 2015
...
namespace Project4Starter
{
    ...
    public class Receiver
    {
        ...
        public Receiver(string Port, string Address)
        {
            address = Address;
            port = Port;
        }

        public Receiver(string Port, string Address, HiResTimer timer)
        {
            address = Address;
            port = Port;
            this.timer = timer;
        ...
        public Action defaultServiceAction()
        {
            Action serviceAction = () =>
            {
                ...
                while (true)
                {
                    ...
                    timer.Stop();
                    totalExecutionTime = timer.ElapsedTimeSpan.TotalSeconds;
                    string receivedMsg = " Client: Avg. Latency = " + totalExecutionTime / (++count) + " secs; Total response received = " + count;
                    Console.Title = receivedMsg;
