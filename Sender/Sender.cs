 ...
 * --------------------
 * ver 2.2 : 18 Nov 2015
 * - Added new methods to serve performance metrics transportation
 * ver 2.1 : 29 Oct 2015
    ...
    public class Sender
    {
        ...
        public PerformanceDetails GetPerformanceDetails()
        {
            if (proxyStore.ContainsKey(remoteUrl))
            {
                ...
                if (Util.verbose)
                    Console.Write("\n  sender sending message to service {0}", remoteUrl);
                return proxyStore[remoteUrl].getPerformanceDetails();
            }
            else
            {
                ...
                {
                    if (Util.verbose)
                        Console.Write("\n  sender created proxy and sending message {0}", remoteUrl);
        ...
        public void SendReaderLatency(double time)
        {
            if (proxyStore.ContainsKey(remoteUrl))
            {
                ...
                if (Util.verbose)
                    Console.Write("\n  sender sending message to service {0}", remoteUrl);
                proxyStore[remoteUrl].sendReaderLatency(time);
            }
            else
            {
                // create new proxy with Connect, save it, and use it
        ...
        public void SendWriterLatency(double time)
        {
            if (proxyStore.ContainsKey(remoteUrl))
            {
                ...
                if (Util.verbose)
                    Console.Write("\n  sender sending message to service {0}", remoteUrl);
                proxyStore[remoteUrl].sendWriterLatency(time);
            }
            else
            {
                // create new proxy with Connect, save it, and use it
        ...
        /// <summary>
        /// Sends the server throughput.
        /// </summary>
        /// <param name="time">The time.</param>
        public void SendServerThroughput(double time)
        {
            if (proxyStore.ContainsKey(localUrl))
            {
                // proxy already created so use it
                if (Util.verbose)
                    Console.Write("\n  sender sending message to service {0}", remoteUrl);
                proxyStore[remoteUrl].sendThroughputDetails(time);
            }
            else
            {
                // create new proxy with Connect, save it, and use it
                proxyStore[remoteUrl] = this.CreateProxy(localUrl);  // save proxy
                proxyStore[remoteUrl].sendThroughputDetails(time);
            }
        }
